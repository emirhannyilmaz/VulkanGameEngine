#include "vertical_gaussian_blur_post_processing.hpp"

VerticalGaussianBlurPostProcessing::VerticalGaussianBlurPostProcessing(Renderer* renderer) {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vertexUniformBuffers[i] = new Buffer(renderer->device->physicalDevice, renderer->device->device, sizeof(VerticalGaussianBlurVertexUniformBufferObject), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        descriptorSets->updateBufferInfo(i, 0, 0, 1, vertexUniformBuffers[i]->buffer, sizeof(VerticalGaussianBlurVertexUniformBufferObject));
...
void VerticalGaussianBlurPostProcessing::CreateGraphicsPipeline() {
...
void VerticalGaussianBlurPostProcessing::process(float imageHeight, CommandBuffers* commandBuffers) {
    updateDescriptorSetResources(imageHeight);
...
void VerticalGaussianBlurPostProcessing::updateDescriptorSetImageInfos() {
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        descriptorSets->updateImageInfo(i, 1, 0, 1, renderer->gaussianBlurResources->horizontalBlurColorResources->image->imageView, renderer->gaussianBlurResources->sampler->sampler);
    }
}

void VerticalGaussianBlurPostProcessing::updateDescriptorSetResources(float imageHeight) {
    VerticalGaussianBlurVertexUniformBufferObject vertexUbo{};
    vertexUbo.imageHeight = imageHeight;
    void* vubData;
