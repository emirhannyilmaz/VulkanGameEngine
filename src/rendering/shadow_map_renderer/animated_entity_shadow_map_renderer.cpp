#include "animated_entity_shadow_map_renderer.hpp"

AnimatedEntityShadowMapRenderer::AnimatedEntityShadowMapRenderer(Renderer* renderer) {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        vertexUniformBuffers[i] = new Buffer(renderer->device->physicalDevice, renderer->device->device, sizeof(AnimatedEntityShadowMapRendererVertexUniformBufferObject), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
        descriptorSets->updateBufferInfo(i, 0, 0, 1, vertexUniformBuffers[i]->buffer, sizeof(AnimatedEntityShadowMapRendererVertexUniformBufferObject));
    }
}
...
void AnimatedEntityShadowMapRenderer::CreateGraphicsPipeline() {
    ...
    graphicsPipeline = new GraphicsPipeline(renderer->device->device, "res/shaders/animated_entity_shadow_map_shader.vert.spv", "res/shaders/animated_entity_shadow_map_shader.frag.spv", 1, &bindingDescription, static_cast<uint32_t>(attributeDescriptions.size()), attributeDescriptions.data(), static_cast<uint32_t>(descriptorSetLayouts.size()), descriptorSetLayouts.data(), 0, nullptr, renderer->shadowMapExtent, renderer->shadowMapResources->renderPass->renderPass, VK_SAMPLE_COUNT_1_BIT, VK_TRUE, VK_BLEND_FACTOR_ONE_MINUS_SRC_ALPHA);
}

void AnimatedEntityShadowMapRenderer::DeleteGraphicsPipeline() {
    delete graphicsPipeline;
}

void AnimatedEntityShadowMapRenderer::render(std::vector<AnimatedEntity*> entities, Light* light, PerspectiveCamera* perspectiveCamera, OrthographicCamera* orthographicCamera, CommandBuffers* commandBuffers) {
...
void AnimatedEntityShadowMapRenderer::updateDescriptorSetResources(Light* light, PerspectiveCamera* perspectiveCamera, OrthographicCamera* orthographicCamera) {
    AnimatedEntityShadowMapRendererVertexUniformBufferObject vertexUbo{};
