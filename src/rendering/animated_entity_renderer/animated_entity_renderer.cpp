...
void AnimatedEntityRenderer::CreateGraphicsPipelines() {
    ...
    offScreenGraphicsPipeline = new GraphicsPipeline(renderer->device->device, "res/shaders/animated_entity_shader.vert.spv", "res/shaders/animated_entity_shader.frag.spv", 1, &bindingDescription, static_cast<uint32_t>(attributeDescriptions.size()), attributeDescriptions.data(), static_cast<uint32_t>(descriptorSetLayouts.size()), descriptorSetLayouts.data(), static_cast<uint32_t>(pushConstantRanges.size()), pushConstantRanges.data(), renderer->swapchain->swapchainExtent, renderer->waterResources->renderPass->renderPass, VK_SAMPLE_COUNT_1_BIT);
}
...
void AnimatedEntityRenderer::render(std::vector<AnimatedEntity*> animatedEntities, Light* light, PerspectiveCamera* perspectiveCamera, glm::vec4 clipPlane, CommandBuffers* commandBuffers, bool onScreen) {
    ...
    vkCmdPushConstants(commandBuffer, gp->pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(AnimatedEntityRendererVertexPushConstants), &vertexPushConstants);
