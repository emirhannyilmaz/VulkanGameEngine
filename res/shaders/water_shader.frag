...
layout(set = 0, binding = 5) uniform WaterRendererFragmentUniformBufferObject {
    float moveFactor;
    vec3 lightColor;
} wrfubo;
...
void main() {
    ...
    vec2 totalDistortion = (texture(dudvMap, distortedTextureCoordinates).rg * 2.0 - 1.0) * waveStrength;
    ...
    vec3 specularLighting = sDamped * wtfubo.reflectivity * wrfubo.lightColor;
